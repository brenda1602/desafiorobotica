...
#define LIGHT_SENSOR_TRESHOULD 65

//light sensor configuration
WbDeviceTag ls_tag[3];
double light_sensor_value[3]={0.0, 0.0, 0.0};
int count=0;
int find_stop= false;
...
typedef enum { FORWARD, LEFT, RIGHT, FINISH } State;
...
int main() {
  ...
  printf("Looking for the stop sign...\n");
  ...
  //set up light sensor
  for (i = 0; i < 3; i++){
   sprintf(sensor_name, "lo%d", i);
   ls_tag[i]=wb_robot_get_device(sensor_name);
   wb_light_sensor_enable(ls_tag[i],time_step);
  }
  ...
  // run simulation
  while ((wb_robot_step(time_step) != -1) && !find_stop) {
      ...
      //find the stop sign condition
      if (count>300){
        find_stop=true;
        printf("Find the stop sign\n");
        state= FINISH;
      }
      ...
      switch (state) {
        ...
        case FORWARD:
          if (wheel_weight_total[0] > WHEEL_WEIGHT_THRESHOLD && !find_stop) {
            speed[0] = 0.9 * MAX_SPEED;
            speed[1] = -0.9 * MAX_SPEED;
            state = LEFT;
          } else if (wheel_weight_total[1] > WHEEL_WEIGHT_THRESHOLD && !find_stop) {
            speed[0] = -0.9 * MAX_SPEED;
            speed[1] = 0.9 * MAX_SPEED;
            state = RIGHT;
          } else {
        ...
        case LEFT:
          if ((wheel_weight_total[0] > WHEEL_WEIGHT_THRESHOLD || wheel_weight_total[1] > WHEEL_WEIGHT_THRESHOLD)&& !find_stop) {
            speed[0] = 0.9 * MAX_SPEED;
            speed[1] = -0.9 * MAX_SPEED;
          } else {
            speed[0] = MAX_SPEED;
      ...
      // check light sensors 
      for ( i = 0; i < 3; i++){
        light_sensor_value[i]= wb_light_sensor_get_value(ls_tag[i]);
      }
      //directs movements in favor of light
      if(light_sensor_value[1]>LIGHT_SENSOR_TRESHOULD && light_sensor_value[2]>LIGHT_SENSOR_TRESHOULD && !find_stop){
          speed[0] = MAX_SPEED;
          speed[1] = MAX_SPEED;
      } else if(light_sensor_value[2]>LIGHT_SENSOR_TRESHOULD && !find_stop){
          speed[0] = 0.7 * MAX_SPEED;
          speed[1] = -0.7* MAX_SPEED;
      } else if(light_sensor_value[1]>LIGHT_SENSOR_TRESHOULD && !find_stop){
          speed[0] = -0.7 * MAX_SPEED;
          speed[1] = 0.7 * MAX_SPEED;
      }
      // when the robot arrives in the light zone
      if(light_sensor_value[0]>LIGHT_SENSOR_TRESHOULD && light_sensor_value[1]>LIGHT_SENSOR_TRESHOULD && light_sensor_value[2]>LIGHT_SENSOR_TRESHOULD ){
        count+=1;
      }
